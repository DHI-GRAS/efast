...
from efast.binning import BBox, SynProduct, bin_to_grid, create_geogrid, get_reflectance_filename, bin_to_grid_numpy
...
def test_bin_to_grid_dev():
    ...
    band_names = [*reflectance_bands, "CLOUD_flags"]
    prod = SynProduct(SYN_PRODUCT_PATH, band_names=band_names)
    ds = prod.read_bands()
    ...
    with rasterio.open(TEST_DATA_OUTPUT_PATH / "binned.tif", "w", driver="GTiff", height=binned.shape[1], width=binned.shape[2], count=len(reflectance_bands), dtype=binned.dtype, crs="+proj=latlong", transform=transform) as ds:
        ds.write(binned[:, ::-1, :])
